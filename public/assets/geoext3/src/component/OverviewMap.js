...
Ext.define('GeoExt.component.OverviewMap', {
    ...
    initOverviewMap: function() {
        ...
        if (me.getLayers().length < 1) {
            ...
            if('overviewLayers' in me.config){
                if(me.config.overviewLayers.length > 0){
                    me.getLayers().push(me.config.overviewLayers[0]);
                }
            }else{
                parentLayers.forEach(function(layer) {
                    if(layer.get('id') == "basic_group"){
                        for (var i = 0; i < layer.get('layers').getArray().length; i++) {
                            if (layer.get('layers').getArray()[i] instanceof ol.layer.Tile ||
                                layer.get('layers').getArray()[i] instanceof ol.layer.Image) {
                                me.getLayers().push(layer.get('layers').getArray()[i]);
                            }
                        };
                    }else{
